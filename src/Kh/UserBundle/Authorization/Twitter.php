...
class Twitter extends OAuthAbstract
{
	...
	{
		if ($this->getSettingsManager()->get('social.twitter_consumer_key') && $this->getSettingsManager()->get('social.twitter_consumer_secret')) {
			$connection = new \TwitterOAuth(
				$this->getSettingsManager()->get('social.twitter_consumer_key'), $this->getSettingsManager()->get('social.twitter_consumer_secret'));
			$request_token = $connection->getRequestToken($this->generateUrl('_twitter_authorization_redirect', array(), true) . $this->getBackUrl());
			$this->getSession()->set('twitter_request_token', $request_token);
	...
	protected function findUser()
	{
		...
		return $this->getUserManager()->getUserByTwitterId($this->data->id);
	}

	protected function getData()
	{
		if ($this->data === null) {
			$request_token = $this->getSession()->get('twitter_request_token');
			$connection = new \TwitterOAuth($this->getSettingsManager()->get('social.twitter_consumer_key'), $this->getSettingsManager()->get('social.twitter_consumer_secret'),
				$request_token['oauth_token'], $request_token['oauth_token_secret']);
			$accessToken = $connection->getAccessToken($this->getRequest()->query->get('oauth_verifier'));
			$this->getSession()->uns('twitter_request_token');
			$connection = new \TwitterOAuth($this->getSettingsManager()->get('social.twitter_consumer_key'), $this->getSettingsManager()->get('social.twitter_consumer_secret'),
