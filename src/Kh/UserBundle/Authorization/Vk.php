...
class Vk extends OAuthAbstract
{
	...
	{
		$params = array(
			'client_id' => $this->getSettingsManager()->get('social.vk_appid'),
			'scope' => '',
	...
	protected function findUser()
	{
		...
		return $this->getUserManager()->getUserByVkId($this->token['user_id']);
	}
	...
	protected function getToken()
	{
		if (!$this->token) {
			...
			$params = array(
				'client_id' => $this->getSettingsManager()->get('social.vk_appid'),
				'client_secret' => $this->getSettingsManager()->get('social.vk_secret'),
				'code' => $code,
				'redirect_uri' => $this->generateUrl('_vk_redirect', array(), true) . $this->getBackUrl(),
			);
